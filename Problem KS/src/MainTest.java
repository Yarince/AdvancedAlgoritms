...
public class MainTest {
    ...
    public static Collection input() {
        ...
        try {
            for (int i = 1; i < 8; i++) {
                String input = new String(Files.readAllBytes(Paths.get(String.format("%s/src/ks/90%s.in", filePath, i))), StandardCharsets.UTF_8);
                String output = new String(Files.readAllBytes(Paths.get(String.format("%s/src/ks/90%s.out", filePath, i))), StandardCharsets.UTF_8);
